...
if (clase_generar == 0):
  index_generar = np.random.choice(index_adidas[0], size=num_elementos_generar)
elif (clase_generar == 1):
  index_generar = np.random.choice(index_converse[0], size=num_elementos_generar)
elif (clase_generar == 2):
  index_generar = np.random.choice(index_nike[0], size=num_elementos_generar)
else:
...
label = np.array([clase_generar]).reshape((1,1))
img_gen = []
...
filename_gen = etiquetas[clase_generar]+"_"+time_now.strftime("%Y-%m-%d_%H-%M-%S")+".png"
path_data_aumentation = 'data_aumentation/'+filename_gen

#cv.imwrite(path_data_aumentation, img_resize[0])

#... get array s.t. arr.shape = (3,256, 256)
#img = Image.fromarray(arr, 'RGB')
#img.save('out.png')

data = im.fromarray(img_resize[1], 'RGB')
data.save(path_data_aumentation)